...
class LocalPartyReport extends StatefulWidget {
  ...
  LocalPartyReport({this.partyName, this.docID, this.defaultDisc, this.invoiceDate, this.invoiceAmount});
...
class _LocalPartyReportState extends State<LocalPartyReport> with SingleTickerProviderStateMixin {
  Map<String, ProductData> m = Map();

  double finalAmount=0;

  firestore(){
    Firestore.instance.collection('Expiry').document(widget.docID).collection(widget.partyName).snapshots().listen(
  ...
  List<List<String>> trial = List();
   var element = (["S.No.", "Product" , "Pack" , "QTY", "Expiry Dt." ,"Deal", "MRP"]);
  int a=0;
  
    m.forEach((k,v){
      trial.add([v.name,v.pack.toString(),v.batchNumber,v.expiryDate,v.qty.toString(),v.deal1.toString() + " + " + v.deal2.toString(),v.mrp.toString()]);
      a++;
    });
    ...
    print(trial);
    const tableHeaders = ["S.No.", "Product" , "Pack" ,"Batch No.", "Expiry Dt." , "QTY", "Deal", "MRP"];
  ...
  doc.addPage(
    pw.MultiPage(
      ...
      build: (pw.Context context) => [
          pw.Table.fromTextArray(context: context, data: trial, cellAlignment: pw.Alignment.topLeft, headers: List<String>.generate(
        tableHeaders.length,
      ...
      pw.Container(
          ...
          child: pw.Row(children: <pw.Widget>[
            pw.Container(padding: pw.EdgeInsets.all(12.0),
              ...
              pw.Text("DT - " + widget.invoiceDate.toDate().day.toString() + "/" + widget.invoiceDate.toDate().month.toString() + "/" + widget.invoiceDate.toDate().year.toString(), textAlign: pw.TextAlign.right  ,style: pw.TextStyle(font: pw.Font.times(),fontWeight: pw.FontWeight.normal, fontSize: 10))
            ])),
              ...
              pw.Padding(child: pw.Text("  M/S " + widget.partyName, textAlign: pw.TextAlign.right  ,style: pw.TextStyle(font: pw.Font.times(),fontWeight: pw.FontWeight.normal, fontSize: 13)),
