...
class _ProdSearchState extends State<ProdSearch> {
  ...
  discountList(){
    Firestore.instance.collection("Party").snapshots().listen((event) => event.documents.forEach((element) { 
   ...
   fillData(String prodName){
    ...
    Firestore.instance.collection('Expiry').snapshots().listen((event) => event.documents.forEach((element) {
      Firestore.instance.collection('Expiry').document(element.documentID).collection(element['partyName']).where('prodName', isEqualTo: prodName).snapshots().listen(
        (cour)=> cour.documents.forEach((doc) {           
          print("2");
          DocumentID docu = DocumentID();
          docu.docId1     = element.documentID;
          docu.docId2     = doc.documentID;
          docu.date       = element['timestamp'];
          docu.partyName  = element['partyName'];
          //docu.discount   = Firestore.instance.collection("Party").where('partyName', isEqualTo: docu.partyName).getDocuments()
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: new AppBar(
            ...
            title: new Text(widget.prodName, style: TextStyle(color: Colors.white, fontSize: 22.0, fontWeight: FontWeight.w600),), 
          ),
                      ...
                      child: ListTile(
                        ...
                        leading: Container(
                          ...
                          child: IconButton(icon: Icon(Icons.edit), iconSize: 25 ,onPressed: (){
                            PartyData data = PartyData(name: docu.partyName, defaultDiscount: m[docu.partyName]);
                            Navigator.push(context, 
                              MaterialPageRoute(
                                builder: (context) => AllProductPage(data: data, docID: docu.docId1, check: true, invoiceDate: docu.date,invoiceAmount: docu.amt)));
                          })
        ),
         title: Flexible(
                    child: Text('${docu.partyName}',overflow: TextOverflow.ellipsis,
        ...
        subtitle: Column(
          ...
          children: <Widget>[
            ...
            Text('${prod.pack}', style: TextStyle(color: Colors.black)),
            SizedBox(width: 5),
            Text("Expiry Date: ", style: TextStyle(color: Colors.black,fontWeight: FontWeight.bold)),
            Text('${prod.expiryDate}', style: TextStyle(color: Colors.black)),
        ...
        Row(
          children: <Widget>[
            ...
            Text('${prod.batchNumber}', style: TextStyle(color: Colors.black)),
            SizedBox(width: 5),
            Text("MRP: ", style: TextStyle(color: Colors.black,fontWeight: FontWeight.bold)),
            Text('\u20B9 ${prod.mrp}', style: TextStyle(color: Colors.black)),
        ...
        Row(
          children: <Widget>[
            ...
            Text('${prod.qty}', style: TextStyle(color: Colors.black)),
            SizedBox(width: 5),
            Text("Deal: ", style: TextStyle(color: Colors.black,fontWeight: FontWeight.bold)),
            Text('${prod.deal1}+${prod.deal2}', style: TextStyle(color: Colors.black))         
            ],
        ),
          ]
        ),
         trailing: Container(
           child:
            ...
            Text('${docu.date.toDate().day}/${docu.date.toDate().month}/${docu.date.toDate().year}', style: TextStyle(color: Colors.black, fontSize: 20)),
             ])
             )
